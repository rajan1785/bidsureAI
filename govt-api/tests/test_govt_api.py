...
def test_gst_known_gstin_active():
    r = client.get("/api/v1/gstn/verify/07AAECS1234F1Z5")
...
def test_gst_unknown_gstin_404():
    r = client.get("/api/v1/gstn/verify/29ZZZZZ9999Z1Z9")
    assert r.status_code == 404
    assert r.json()["error"]["code"] == "RECORD_NOT_FOUND"


def test_pan_valid():
    r = client.get("/api/v1/pan/verify/AAFCN5678K")
    assert r.status_code == 200
    assert r.json()["data"]["status"] == "Valid"


def test_blacklist_hit_for_bidder_c():
    r = client.get("/api/v1/debarment/check/AAKCA9012M")
    assert r.status_code == 200
    data = r.json()["data"]
...
def test_blacklist_clean_for_bidder_a():
    r = client.get("/api/v1/debarment/check/AAECS1234F")
    assert r.status_code == 200
    assert r.json()["data"]["clean"] is True


def test_epfo_bidder_b_expired_validity():
    r = client.get("/api/v1/epfo/establishment/DLCPM0023456000")
    assert r.status_code == 200
